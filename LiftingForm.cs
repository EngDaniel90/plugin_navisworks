...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // Cache COM objects outside the loop to prevent O(N) marshalling overhead
            ...
            try
            {
                ...
                {
                    await Task.Yield(); // Maximize throughput while yielding to UI
                    Application.DoEvents(); // Força a renderização da tela e da geometria
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            break; // Stop checking further results once a valid collision is found
                        }
                    }

                    if (collisionDetected)
                    {
                        break; // Exit the descent loop immediately to prevent redundant clash tests
                    }
