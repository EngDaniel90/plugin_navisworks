...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // ⚡ Bolt: Cache COM objects outside the simulation loop.
            // Converting .NET selection to COM (ToInwOpSelection) is O(N) over items.
            // Caching this prevents repetitive marshalling overhead per frame.
            ComApi.InwOpState10 cachedState = ComApiBridge.State;
            ComApi.InwOpSelection cachedSelection = ComApiBridge.ToInwOpSelection(items);
            ...
            try
            {
                ...
                {
                    // ⚡ Bolt: Replace Task.Delay(50) with Task.Yield() + Application.DoEvents().
                    // Maximize simulation throughput while keeping UI responsive and forcing view rendering.
                    await Task.Yield();
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            // ⚡ Bolt: Early exit from results check loop once collision is found
                            break;
                        }
                    }
