...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async void btnRun_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (SavedItem module in modules)
                {
                    ...
                    // 3. RODAR SIMULAÇÃO
                    // ⚡ Bolt: Pass TestsData directly to avoid redundant doc.GetClash().TestsData calls inside simulation
                    bool collisionFound = await RunSimulation(doc, testsData, itemsToMove, activeTest, userHeightMeters, userStepMeters);
        ...
        private async Task<bool> RunSimulation(Document doc, DocumentClashTests clashTestsData, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            double currentZMeters = heightMeters;
            bool collisionDetected = false;

            // ⚡ Bolt: Cache COM objects once to avoid O(N) marshalling overhead per frame
                ...
                while (currentZMeters >= 0)
                {
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            // ⚡ Bolt: Early exit on inner validation loop
                            break;
                        }
                    }

                    // ⚡ Bolt: Early exit on outer simulation loop if collision found
