...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // ⚡ BOLT OPTIMIZATION: Cache COM API objects outside the simulation loop.
            // Converting .NET selection to COM selection (`ToInwOpSelection`) is an expensive O(N) operation
            // doing this repeatedly inside the loop causes massive overhead for large selections.
                ...
                while (currentZMeters >= 0)
                {
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            // ⚡ BOLT OPTIMIZATION: Early exit - once a clash is found, there is no need to keep checking
                            // down the rest of the height.
                            break;
                        }
                    }
