...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async void btnRun_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (SavedItem module in modules)
                {
                    ...
                    await Task.Yield(); // ⚡ Bolt: Yield instead of Delay to maintain throughput
                    System.Windows.Forms.Application.DoEvents(); // ⚡ Bolt: Force Navisworks to render geometry without blocking the thread
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // ⚡ Bolt: Cache COM objects outside the loop to prevent O(N) marshalling overhead
            ...
            try
            {
                ...
                {
                    await Task.Yield(); // ⚡ Bolt: Yield instead of Delay to maintain throughput
                    System.Windows.Forms.Application.DoEvents(); // ⚡ Bolt: Force Navisworks to render geometry without arbitrarily waiting
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            break; // ⚡ Bolt: Stop checking children on first collision
                        }
                    }

                    if (collisionDetected) break; // ⚡ Bolt: Short-circuit simulation loop on collision
