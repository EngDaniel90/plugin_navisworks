...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async void btnRun_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (SavedItem module in modules)
                {
                    ...
                    // 3. RODAR SIMULAÇÃO
                    // ⚡ Bolt: Cache COM State and Selection outside the simulation loop to avoid O(N) marshalling overhead per step
                    ComApi.InwOpState10 cachedState = ComApiBridge.State;
                    ComApi.InwOpSelection cachedSelection = ComApiBridge.ToInwOpSelection(itemsToMove);

                    bool collisionFound = await RunSimulation(doc, activeTest, userHeightMeters, userStepMeters, cachedState, cachedSelection);
        ...
        private async Task<bool> RunSimulation(Document doc, ClashTest test, double heightMeters, double stepMeters, ComApi.InwOpState10 state, ComApi.InwOpSelection comSelection)
        {
                ...
                while (currentZMeters >= 0)
                {
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            // ⚡ Bolt: Early exit from validation loop to save processing time
                            break;
                        }
                    }

                    // ⚡ Bolt: Early exit from simulation loop to prevent expensive TestsRunTest executions
