...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async void btnRun_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (SavedItem module in modules)
                {
                    ...
                    await Task.Yield();
                    System.Windows.Forms.Application.DoEvents(); // Otimizado para renderizar a tela com maior throughput
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // Cache COM objects before the simulation loop to avoid O(N) marshalling overhead
            ...
            try
            {
                ...
                {
                    await Task.Yield();
                    System.Windows.Forms.Application.DoEvents(); // Maximizes throughput while allowing Navisworks view updates
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            break; // Early exit: stop checking remaining clash results
                        }
                    }

                    if (collisionDetected)
                    {
                        break; // Early exit: stop running subsequent clash tests for this module
                    }
