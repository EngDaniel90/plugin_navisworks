...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // Reduces O(N) marshalling overhead per step
            ComApi.InwOpState10 cachedState = null;
            ComApi.InwOpSelection cachedComSelection = null;
            try
            {
                cachedState = ComApiBridge.State;
                cachedComSelection = ComApiBridge.ToInwOpSelection(items);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine("Erro COM Init: " + ex.Message);
                return false; // Safely abort if COM bridges fail
            }
                ...
                while (currentZMeters >= 0)
                {
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            // ⚡ Bolt Optimization: Early exit if collision detected
                            break;
                        }
                    }

                    // ⚡ Bolt Optimization: Break the entire loop as no more iterations are needed once collision found
