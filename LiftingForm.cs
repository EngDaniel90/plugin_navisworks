...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // Cache COM objects to avoid O(N) marshalling overhead inside the simulation loop
            ...
            try
            {
                ...
                {
                    await Task.Yield(); // Permite que a UI processe
                    Application.DoEvents(); // Força renderização da view do Navisworks (Maximizando throughput)
