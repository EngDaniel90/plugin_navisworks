...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // ⚡ Bolt: Cache COM objects outside the loop to avoid O(N) marshalling overhead on every step
        ...
        private void MoveItemsUsingCOM(ComApi.InwOpState10 state, ComApi.InwOpSelection selection, double zMeters)
        {
            try
            {
                ...
                state.OverrideTransform(selection, transform);
            }
            catch (Exception ex)
            {
        ...
        private void ResetItemsUsingCOM(ComApi.InwOpState10 state, ComApi.InwOpSelection selection)
        {
            try
            {
