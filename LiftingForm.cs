    ...
    public partial class LiftingForm : Form
    {
        ...
        private async void btnRun_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (folderGroup == null)
                {
                    throw new Exception("O item selecionado não é uma pasta válida.");
                }
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // ⚡ BOLT OPTIMIZATION: Cache COM API bridge states outside the loop to avoid O(N) marshalling overhead
            ComApi.InwOpState10 state = null;
            ComApi.InwOpSelection comSelection = null;
            try
            {
                state = ComApiBridge.State;
                comSelection = ComApiBridge.ToInwOpSelection(items);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine("Erro COM Init: " + ex.Message);
                return false;
            }
                ...
                while (currentZMeters >= 0)
                {
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            // ⚡ BOLT OPTIMIZATION: Early exit da validação assim que uma colisão é confirmada
                            break;
                        }
                    }

                    // ⚡ BOLT OPTIMIZATION: Early exit do loop de simulação principal
            ...
            finally
            {
                ...
                if (state != null && comSelection != null)
                {
                    ResetItemsUsingCOM(state, comSelection);
                }

                // ⚡ BOLT OPTIMIZATION: Compensate for skipped progress bar updates if early exited
                if (collisionDetected)
                {
                    int remainingSteps = (int)(currentZMeters / stepMeters) + 1;
                    if (remainingSteps > 0)
                    {
                        if (progressBar.InvokeRequired)
                        {
                            progressBar.Invoke(new Action(() =>
                            {
                                int newValue = progressBar.Value + remainingSteps;
                                progressBar.Value = Math.Min(newValue, progressBar.Maximum);
                            }));
                        }
                        else
                        {
