...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // ⚡ Bolt: Cache COM state and selection outside the loop to prevent O(N) marshalling overhead
            // Creating COM selections from .NET collections repeatedly is highly inefficient.
                ...
                while (currentZMeters >= 0)
                {
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            // ⚡ Bolt: Early exit strategy. Break inner validation loop upon first valid collision.
                            break;
                        }
                    }

                    // ⚡ Bolt: Early exit from outer simulation loop. No need to test further steps once collided.
        ...
        // ⚡ Bolt: Updated parameters to accept cached COM objects, removing repeated Marshalling
