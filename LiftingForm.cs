...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async void btnRun_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (SavedItem module in modules)
                {
                    ...
                    // 3. RODAR SIMULAÇÃO
                    bool collisionFound = await RunSimulation(doc, itemsToMove, activeTest, userHeightMeters, userStepMeters, testsData);
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters, DocumentClashTests clashTestsData)
        {
            ...
            try
            {
                // Cache COM State and Selection to avoid O(N) marshalling in loops
                ...
                while (currentZMeters >= 0)
                {
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            break; // Parar na primeira batida (early exit)
                        }
                    }

                    if (collisionDetected)
                    {
                        break; // Sai do loop de descida
                    }
            ...
            finally
            {
                ...
                if (ComApiBridge.State != null)
                {
                    ResetItemsUsingCOM(ComApiBridge.State, ComApiBridge.ToInwOpSelection(items));
                }
            }
