...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // ⚡ Bolt: Cache COM objects outside the loop to eliminate O(N) marshalling overhead
            ...
            try
            {
                ...
                {
                    await Task.Yield(); // ⚡ Bolt: Yield instead of hardcoded Delay to maximize throughput
                    Application.DoEvents(); // Força o Navisworks a renderizar a geometria nova
                    ...
                    {
                        break; // ⚡ Bolt: Stop simulation once collision is found
                    }
