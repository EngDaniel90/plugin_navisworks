...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            try
            {
                // Cache COM State and Selection to avoid repeated marshalling overhead (O(N) -> O(1))
                // This optimization is crucial for performance as marshalling is expensive
                state = ComApiBridge.State;
                comSelection = ComApiBridge.ToInwOpSelection(items);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine("Erro COM Setup: " + ex.Message);
                return false; // Cannot proceed without COM selection
            }
