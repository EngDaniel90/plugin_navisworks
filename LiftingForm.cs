...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async void btnRun_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (SavedItem module in modules)
                {
                    ...
                    Application.DoEvents(); await Task.Yield(); // Permite renderização mais rápida e libera a thread do UI sem delay fixo
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // ⚡ Bolt: Cache COM APIs outside the simulation loop to avoid O(N) marshalling penalty on each step.
            ...
            try
            {
                ...
                {
                    Application.DoEvents(); await Task.Yield(); // ⚡ Bolt: Maximiza throughput vs. hardcoded Task.Delay(50)
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            break; // ⚡ Bolt: Parar assim que achar colisão
                        }
                    }

                    if (collisionDetected) break; // ⚡ Bolt: Break the descent loop to skip unnecessary operations.
