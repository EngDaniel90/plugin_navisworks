...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            ComApi.InwOpSelection comSelection;

            try
            {
                // Cache COM API objects to prevent overhead
            ...
            // Pre-allocate delegate to update progress bar, reducing GC pressure
            MethodInvoker incrementProgress = () => { if (progressBar.Value < progressBar.Maximum) progressBar.Increment(1); };
            MethodInvoker updateProgressBar = () =>
            {
                if (progressBar.InvokeRequired)
                {
                    progressBar.Invoke(incrementProgress);
                }
                else
                {
                    incrementProgress();
                ...
                while (currentZMeters >= 0)
                {
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            break; // Early exit from the validation loop once a collision is found
                        }
                    }

                    // Early exit from the simulation if collision detected
