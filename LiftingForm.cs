                ...
                while (currentZMeters >= 0)
                {
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            break; // Early exit do loop de validação
                        }
                    }

                    if (collisionDetected)
                    {
                        break; // Early exit do loop de simulação principal para economizar processamento
                    }
                    ...
                    {
                        progressBar.Invoke(new Action(() => { if (progressBar.Value < progressBar.Maximum) progressBar.Increment(1); }));
                    }
                    else
                    {
