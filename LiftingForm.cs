                ...
                while (currentZMeters >= 0)
                {
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            // ⚡ Bolt: Early exit from the inner loop to save expensive `TestsRunTest` executions
                            break;
                        }
                    }

                    // ⚡ Bolt: Early exit from the main simulation loop if a collision was detected
