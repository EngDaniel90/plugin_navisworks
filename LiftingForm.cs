...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // ⚡ Bolt: Cache COM State and Selection OUTSIDE the main loop.
            // Calling ComApiBridge.ToInwOpSelection(items) is an O(N) operation
            // that was previously done twice per loop iteration. Doing it once saves massive CPU overhead.
            ComApi.InwOpState10 cachedState = ComApiBridge.State;
            ComApi.InwOpSelection cachedComSelection = ComApiBridge.ToInwOpSelection(items);

            try
            {
                // Mover para o TOPO (Posição Inicial)
                MoveItemsUsingCOM(cachedState, cachedComSelection, currentZMeters);
                ...
                while (currentZMeters >= 0)
                {
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            // ⚡ Bolt: Exit early upon finding a collision. Checking further is redundant.
                            break;
                        }
                    }

                    // ⚡ Bolt: Exit the entire loop if a collision is found.
                    // testsData.TestsRunTest() is extremely expensive. Running it after a collision is found wastes CPU.
            ...
            finally
            {
                ...
                ResetItemsUsingCOM(cachedState, cachedComSelection);
            }
