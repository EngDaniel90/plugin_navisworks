...
namespace AutoLiftingClashAnalysis
{
    public partial class LiftingForm : Form
    {
        ...
        private async Task<bool> RunSimulation(Document doc, ModelItemCollection items, ClashTest test, double heightMeters, double stepMeters)
        {
            ...
            // ComApiBridge.ToInwOpSelection is O(N). Doing this once avoids
            // repetitive marshalling overhead inside the simulation loop.
            ...
            try
            {
                ...
                {
                    // ⚡ BOLT OPTIMIZATION: Maximize throughput while ensuring UI responsiveness
                    // Replace blocking Task.Delay(50) with Task.Yield() + Application.DoEvents()
                    // This forces view rendering without wasting 50ms per iteration.
                    await Task.Yield();
                    ...
                    foreach (SavedItem resultItem in test.Children)
                    {
                        ...
                        if (cr != null && (cr.Status == ClashResultStatus.New || cr.Status == ClashResultStatus.Active))
                        {
                            ...
                            break; // ⚡ BOLT OPTIMIZATION: Early exit - stop checking results once a collision is found
                        }
                    }

                    // ⚡ BOLT OPTIMIZATION: Early exit the simulation loop
                    // No need to keep dropping the object and checking clashes if we already hit something
